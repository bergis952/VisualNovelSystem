...
namespace Ink.UnityIntegration {
    ...
    public class InkDefineSymbols : IActiveBuildTargetChanged {
        ...
        static void Add(BuildTargetGroup buildTargetGroup, params string[] defines) {
            ...
            string definesStr = PlayerSettings.GetScriptingDefineSymbols(namedTarget);
            var allDefines = definesStr.Split(DEFINE_SEPARATOR).ToList();
            allDefines.AddRange(defines.Except(allDefines));
            PlayerSettings.SetScriptingDefineSymbols(namedTarget, string.Join(DEFINE_SEPARATOR.ToString(), allDefines.ToArray()));
        }
        ...
        static void Remove(BuildTargetGroup buildTargetGroup, params string[] defines) {
            ...
            PlayerSettings.SetScriptingDefineSymbols(namedTarget, string.Join(DEFINE_SEPARATOR.ToString(), newDefines.ToArray()));
        }
        ...
        static bool Exists(BuildTargetGroup buildTargetGroup, params string[] defines) {
            ...
            string definesStr = PlayerSettings.GetScriptingDefineSymbols(namedTarget);
            var existingDefines = definesStr.Split(DEFINE_SEPARATOR).ToList();
            return defines.All(d => existingDefines.Contains(d));
        }
